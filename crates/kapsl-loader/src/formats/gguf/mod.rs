    ...
    fn value(&mut self, vtype: GgufValueType) -> Result<GgufValue, GgufError> {
        match vtype {
            ...
            GgufValueType::Array => {
                ...
                let elem_type =
                    GgufValueType::from_u32(elem_type_raw).ok_or_else(|| GgufError::Parse {
                        offset: self.pos,
...
impl GgufFile {
    pub fn open(path: &Path) -> Result<Self, GgufError> {
        ...
        for _ in 0..kv_count {
            let key = r.string()?;
            let vtype_raw = r.u32()?;
            let vtype = GgufValueType::from_u32(vtype_raw).ok_or_else(|| GgufError::Parse {
                offset: r.pos,
        ...
        for _ in 0..tensor_count {
            ...
            let dtype_raw = r.u32()?;
            let dtype =
                GgmlType::from_u32(dtype_raw).ok_or(GgufError::UnsupportedQuant(dtype_raw))?;
            let offset = r.u64()?;
    ...
    pub fn extract_config(&self) -> Result<ModelConfig, GgufError> {
        ...
        let max_position_embeddings = self
            .meta_u32(&format!("{arch}.context_length"))
