use super::{
    ModelSubPoolConfig, PerModelShmAllocator, ShmClassBudget, ShmPoolAllocator, SimpleShmAllocator,
    TieredShmAllocator,
};
...
#[test]
fn concurrent_allocations_receive_distinct_slots() {
    const WORKERS: usize = 32;
    let allocator = Arc::new(SimpleShmAllocator::new_with_ttl(
        4096,
        64,
        WORKERS,
    ...
    let workers = (0..WORKERS)
        .map(|_| {
            ...
            thread::spawn(move || {
                start.wait();
                allocator.try_allocate(32).expect("available slot")
            })
        })
        .collect::<Vec<_>>();
    let offsets = workers
