...
impl LlamaContextParams {
    /// Attach an externally-owned Kapsl paged KV pool to this context.
    ///
    /// # Safety
    ///
    /// `pool` must point to a valid `llama_kapsl_kv_pool_desc` that outlives
    /// the created llama context. The descriptor's device pointers and
    /// callbacks must remain valid until the context is freed.
    #[must_use]
    pub unsafe fn with_kapsl_kv_pool_raw(
        mut self,
        pool: *mut llama_cpp_sys_2::llama_kapsl_kv_pool_desc,
        session_id: u64,
    ) -> Self {
        self.context_params.kapsl_kv_pool = pool;
        self.context_params.kapsl_session_id = session_id;
        self
    }
